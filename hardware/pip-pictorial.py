...
# Pi Zero 2 W brain on UART0: GP0 TX -> Zero RXD (header 10), GP1 RX <- Zero TXD (header 8), GND (header 6)
zero = ic("zero", ["RXD GPIO15 pin 10", "TXD GPIO14 pin 8", "GND pin 6"], "right", (-16, 10.6),
          "Raspberry Pi Zero 2 W, 40-pin header\nthe brain, wire link (3.3 V, no level shifter)", w=5.2)
route(pico[f"p{P('UART_TX')}"], zero.RXD_GPIO15_pin_10, -7.2, OG)
route(pico[f"p{P('UART_RX')}"], zero.TXD_GPIO14_pin_8, -7.6, YE)
route(pico.p3, zero.GND_pin_6, -8.0, BK, dot=True)
d.add(elm.Label().at((-16, 8.6)).label("Zero header seen from above, SD card to the left: pin 1 is the corner nearest the SD card,\nodd pins inner row, even pins outer row. 6, 8, 10 are the 3rd to 5th pins of the outer row.", fontsize=7.5))
